...
- **Comment loop (`list_comments` / `add_comment` / `update_comment`):** agents could write comments long before they could read them, which made the one human↔agent handoff surface write-only. All three tools call the same cores as the REST routes (`listCommentsCore`, `createCommentCore`, `patchCommentCore` in `comments.go`), so the editor+ gate, the anchor rules and the author-only body edit are single-sourced. Three things to know: (1) **`list_comments` is read-scoped** — comment bodies are ordinary member content, unlike `list_shares` whose tokens are bearer secrets, so the role gate does the work; (2) **the `since` cursor is an opaque `"<updated_at>|<zero-padded id>"` key** (`commentActivityKey`, and `sqlCommentActivity` for the space query) that folds in the newest REPLY — so a thread someone answered resurfaces for a poller even though the root row never changed, which an id-only or `created_at` cursor cannot do; (3) **space scope returns roots only** with `reply_count` + `page_title` (an inbox, not a transcript). `update_comment` maps the REST 409 `comment_already_resolved` to success — a duplicate resolve is a retry landing twice for an agent, not a lost race, and the tool declares `IdempotentHint`. Deliberately **no `delete_comment`**: an agent that can silently remove human feedback is a worse product than one that cannot. **Anchor gotcha:** anchors are captured against the editor's PLAIN-TEXT projection (`frontend/src/lib/comments/anchor.ts` — `doc.textBetween(…, '\n')`, inline markdown markers stripped, 32-char context window), *not* the markdown `get_page` returns, so `anchor_exact` will not verbatim-match a body containing inline formatting. Both tool descriptions say so; there is no server-side resolver because reproducing that projection in Go exactly is not possible, and a wrong `anchor_resolved` flag is worse than none.
