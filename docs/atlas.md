...
- **A git credential's username is host-supplied when the credential doesn't carry one** (`defaultGitUser`, `source/git/git.go`). With no username `authURL` puts the token alone in the userinfo — which GitHub accepts for **classic** `ghp_` tokens but **rejects for fine-grained `github_pat_*`**: git then prompts for the missing password half and, with no tty, fails `could not read Password … No such device or address`. **That reads like an expired token and gets misattributed** — the fix is the username, not a new token, so check `meta_json` before re-issuing anything. github.com → `x-access-token`, gitlab.com → `oauth2`; **an unknown host deliberately keeps the old token-as-userinfo form**, because guessing wrong breaks a working self-hosted remote and stays invisible until someone reads `probe_error`. Those set `meta.username` explicitly. Fixed 2026-09-01, along with the two client-side halves that made it unreachable: the credentials dialog rendered a username input **only for jira**, hardcoding `meta: undefined` for every git credential, and it sent jira's value under `username` while the API requires `meta.email` — so *every* Jira credential created in the UI 400'd since launch. Lesson: the create form is the only way most credentials are ever made; a field the API needs but the form cannot express is indistinguishable from the feature being broken.
