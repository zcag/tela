...
> ⚠️ **An experimental changeset is currently on `main`:** [atlas — output-budget changeset](atlas-output-budget-experiment.md) (2026-09-01). Reference pages were being silently truncated at the model's output cap — the SerpApi run lost 19 CLI flags and all 36 environment variables to it — so batches are now sized by what a call can *answer*, `finish_reason` is read instead of discarded, reference items are grounded by their own `file:line` instead of a concatenated-name retrieval query, and the repair loop only keeps a pass that improved coverage. **Measured on SerpApi run 134 vs 133 (same surface): surface coverage 72% → 100%, must-cover 80% → 100%, env vars documented 0/36 → 36/36, at flat cost** (+6 chat calls, fewer prompt tokens, +1m total — the extra drafting is offset by repair no longer running). Marked, single-commit, `git revert`-able; read that doc before changing anything in the draft/repair path.
